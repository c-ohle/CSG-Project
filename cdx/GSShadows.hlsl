...
void main(triangleadj GS_P a[6], inout TriangleStream<PS_P> stream)
{
	...
	if(dot(N, g_vLightDir.xyz) >= 0.0f)
	{
		PS_P pp[6];  float f = 1.0f / g_vLightDir.z;
		pp[0].P = mul(float4(a[0].P, 1), g_mViewProjection); pp[3].P = mul(float4(a[0].P + g_vLightDir.xyz * ((g_vLightDir.w - a[0].P.z) * f), 1), g_mViewProjection);
		pp[1].P = mul(float4(a[2].P, 1), g_mViewProjection); pp[4].P = mul(float4(a[2].P + g_vLightDir.xyz * ((g_vLightDir.w - a[2].P.z) * f), 1), g_mViewProjection);
		pp[2].P = mul(float4(a[4].P, 1), g_mViewProjection); pp[5].P = mul(float4(a[4].P + g_vLightDir.xyz * ((g_vLightDir.w - a[4].P.z) * f), 1), g_mViewProjection);
