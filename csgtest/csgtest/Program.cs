...
namespace csgtest
{
  ...
  static class CSG
  {
    [ComImport, Guid("96b75f96-e933-4eff-a6a2-ac2aceb1b824")]
    class CTesselator { }
  ...
  static unsafe class GLU
  {
    ...
    public class Tess : CSG.ITesselator
    {
      ...
      public void CorrectTJunctions()
      {
        ...
        //var nt = 0; var tt = ii + ni; memset(tt, 0, (void*)(np * sizeof(int)));
        //for (int i = 0; i < ni; i++) tt[ii[i]] = 1;
        //for (int i = 0; i < np; i++)
        //{
        //  if (tt[i] == 0) continue;
        //  if (nt != i) pp[nt] = pp[i]; tt[i] = nt++;
        //}
        //if (nt != np)
        //{
        //  var ll = ii; ii = (int*)(this.pp + (this.np = nt));
        //  for (int i = 0; i < ni; i++) ii[i] = tt[ll[i]];
        //  this.ii = ii; stack = (byte*)(ii + ni);
        //}
        int rest;
        for (int loop = 0; ; loop++)
        {
